> **Fork note (felipefontoura):** patched the transcript scraper for `youtube-transcript-api>=1.0.0`.
> Upstream 0.1.0 declared `>=1.0.0` but still called the removed 0.6.x `YouTubeTranscriptApi.list_transcripts()` /
> dict-style snippets, so `youtube_get_transcript` always errored. Now uses `YouTubeTranscriptApi().list()` +
> `.fetch().to_raw_data()`. See `src/youtube_mcp/tools/transcripts.py`.
